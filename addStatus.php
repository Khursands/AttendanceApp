...
{
    $obj = new GM_HR\LeaveStatus;
    $obj->Status = $_POST["Status"];
    $obj->ID = isset($_POST["ID"]) ? $_POST["ID"] : 0;

    if (intval($obj->ID) > 0) {
        GM_HR\LeaveStatusDAL::update($security->conn, $obj);
        GM_HR\Common::jsonSuccess(array("Data Updated" => $obj));

    } 
    else {
        GM_HR\LeaveStatusDAL::create($security->conn, $obj);
